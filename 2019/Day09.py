...
def em_output(code, index, base, par_str):
    if (par_str[0] == '0'):
        arg_0 = referencedValue(code, index + 1)
    elif (par_str[0] == '2'):
        arg_0 = relativeValue(code, index + 1, base)
...
def em_input(code, index, base, par_str):
    if (par_str[2] =='0'):
        writeReference(code, index + 3, input())
    else:
        writeRelative(code, index + 3, base, input())
    return (index + 2), base
...
def emulate(code, index, base):
    ...
    if (op == 99):
        ...
        index, base = em_output(code, index, base, par_str)
    elif (op == 3):

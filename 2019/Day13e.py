...
def em_input(code, index, base, par_str, screen):
    #rasterize(screen)

    if (par_str[2] =='0'):
        writeReference(code, index + 1, 0)
    else:
        writeRelative(code, index + 1, base, 0)
...
def emulate(code, index, base, screen):
    ...
    if (op == 99):
        ...
        index, base = em_input(code, index, base, par_str, screen)
    elif (op == 2):
